...
module.exports = async (ctx) => {
    ...
    const wbiInfoQueryStr = utils.generateWbiInfoQueryStr(['w_webid', 'w_rid', 'wts'], `mid=${uid}&ps=30&tid=0&pn=1&keyword=&order=pubdate&platform=web&web_location=1550101&order_avoided=true&${dmInfoQueryStr}`, wbiImg);
    ...
    const getPage = async (pageId) => {
        ...
        const dmInfoQueryStr = utils.generateDmInfoQueryStr(['dm_img_list', 'dm_img_str', 'dm_cover_img_str', 'dm_img_inter'], config.bilibili.webglStr, config.bilibili.webglVendorAndRenderer, utils.generateLogStack());
        const wbiInfoQueryStr = utils.generateWbiInfoQueryStr(['w_webid', 'w_rid', 'wts'], `mid=${uid}&ps=30&tid=0&pn=${pageId}&keyword=&order=pubdate&platform=web&web_location=1550101&order_avoided=true&${dmInfoQueryStr}`, wbiImg);
